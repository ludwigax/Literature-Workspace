...
from .database import worker_engine, worker_session_factory
...
async def run() -> None:
    ...
    if settings.provider == "openai":
        ...
        provider = FakeResponsesProvider(
            prefix=settings.fake_response_prefix,
            delay_seconds=settings.fake_stream_delay_seconds,
        )
    ...
    try:
        while True:
            while len(running) < settings.worker_max_concurrency:
