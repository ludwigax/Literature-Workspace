...
class TurnExecutor:
    def __init__(
        ...
        *,
        ...
        self.tool_registry = tool_registry or build_tool_registry(settings, session_factory)
    ...
    async def claim_next_turn(self) -> uuid.UUID | None:
        async with self.session_factory() as session, session.begin():
            ...
            for candidate in candidates:
                ...
                if running_count < self.settings.chat_principal_max_concurrency:
                    turn = candidate
                    break
            if turn is None:
                return None
            ...
            turn.lease_expires_at = timestamp + timedelta(
                seconds=self.settings.chat_turn_lease_seconds
            )
